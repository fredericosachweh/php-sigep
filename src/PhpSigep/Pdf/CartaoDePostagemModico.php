...
class CartaoDePostagemModico
{
    ...
    public function __construct($plp, $idPlpCorreios, $logoFile, $chancelas = array())
    {
        ...
        foreach ($chancelas as $chancela) {
            switch ($chancela) {
                case CartaoDePostagemModico::TYPE_CHANCELA_CARTA:
                case CartaoDePostagemModico::TYPE_CHANCELA_CARTA_2016:
                    $this->layoutCarta = $chancela;
                    break;
                case CartaoDePostagemModico::TYPE_CHANCELA_SEDEX:
                case CartaoDePostagemModico::TYPE_CHANCELA_SEDEX_2016:
                    $this->layoutSedex = $chancela;
                    break;
                case CartaoDePostagemModico::TYPE_CHANCELA_PAC:
                case CartaoDePostagemModico::TYPE_CHANCELA_PAC_2016:
            ...
            for ($area = 0; $area < 1; $area++) {
                ...
                switch ($servicoDePostagem->getCodigo()) {
                    ...
                    case ServicoDePostagem::SERVICE_PAC_CONTRATO_AGENCIA_PAGAMENTO_NA_ENTREGA_LM:
                        if ($this->layoutPac === CartaoDePostagemModico::TYPE_CHANCELA_PAC) {
                    ...
                    case ServicoDePostagem::SERVICE_SEDEX_CONTRATO_AGENCIA_03050:
                        if ($this->layoutSedex === CartaoDePostagemModico::TYPE_CHANCELA_SEDEX) {
                    ...
                    case ServicoDePostagem::SERVICE_CARTA_COM_A_FATURAR_SELO_E_SE:
                        if ($this->layoutCarta === CartaoDePostagemModico::TYPE_CHANCELA_CARTA) {
                ...
                $this->pdf->SetXY(0, 30);
                $this->t($this->pdf->w, $nf . "                                  Contrato: 9912499308",1, "L", null);
                $this->pdf->SetXY(0, 33);
                if ($this->getEnvioMesmoDestinatario()){
                	$this->t($this->pdf->w, "$numeroPedido", 1, 'L',  null);
                    $this->t($this->pdf->w, "                IMPRESSO REG MÓDICO      ".'                 Peso(kg): ' . ((float)$objetoPostal->getPeso()), 1, 'C',  null);
                }else{
